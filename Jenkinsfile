pipeline {
    ...
    parameters {
        ...
        string(name: 'STUDENT_NAME', defaultValue: 'Zainab Naeem') //provide your name here, no name, no marks
    ...
    stage('Build') {
            steps {
                ...
                bat '''
                    ...
                    if exist src\\*.js (
                        copy src\\*.js build || echo "JS files copied to build folder"
                    ) else (
                        echo "No JS files found in src folder - continuing anyway"
                    )
